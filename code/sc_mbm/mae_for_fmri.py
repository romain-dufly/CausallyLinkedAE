...
class MAEforFMRICross(nn.Module):
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            # initialize cross_map_in and cross_map_out as identity matrix
            #if m == self.cross_map_in:
            #    nn.init.eye_(m.weight)
            #    if m.bias is not None:
            #        nn.init.constant_(m.bias, 0)
            #elif m == self.cross_map_out:
            #    nn.init.eye_(m.weight)
            #    if m.bias is not None:
            #        nn.init.constant_(m.bias, 0)
            if False:
    ...
    def free_decoder(self, x):
        '''Decode an embedding whose size is any number of patches. Do not use cls token. '''
        # embed tokens
        x = self.decoder_embed(x)
        print('x', x.shape)
        ...
        # add pos embed
        x = x + self.decoder_pos_embed[:, :num_patches, :]
        print('x', x.shape)
