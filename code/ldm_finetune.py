...
def main(config):
    ...
    if config.dataset == 'GOD':
        ...
        fmri_latents_dataset_train, fmri_latents_dataset_test = create_THINGS_dataset(patch_size=config.patch_size, fmri_transform=fmri_transform, image_transform=[img_transform_train, img_transform_test])
        num_voxels = fmri_latents_dataset_train.num_voxels
    else:
